pipeline{
    ...
    stages{
        stage('Checkout'){
            steps{
                git 'https://github.com/appandaraj18/spring-petclinic.git'
            }
        }
        stage('Build'){
            steps{
        ...
        stage('Deploy'){
		 ...
		 sh "sudo docker build . -t appandaraj18/petclinic"
		 sh "sudo docker run -d -p 8091:8080 appandaraj18/petclinic"
